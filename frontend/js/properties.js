  ...
  return `
    <article class="property-card bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm flex flex-col">
      <div class="relative overflow-hidden h-52 flex-shrink-0">
        <img src="${escapeHtml(image)}" alt="${escapeHtml(p.title)}" class="card-img w-full h-full object-cover transition-transform duration-500" />
        ${p.status_id ? `<span class="absolute top-3 left-3 bg-white/90 text-navy text-xs font-semibold px-2.5 py-1 rounded-lg">${escapeHtml(p.status_name || 'Disponible')}</span>` : ''}
        ${p.type_name ? `<span class="absolute top-3 right-3 bg-[#0F1C2E]/80 text-white text-xs font-semibold px-2.5 py-1 rounded-lg">${escapeHtml(p.type_name)}</span>` : ''}
      </div>
      <div class="p-5 flex flex-col flex-1">
        <p class="font-serif font-bold text-[#0F1C2E] text-xl mb-1">${formatPrice(p.price)}</p>
        <h3 class="font-semibold text-[#0F1C2E] text-sm mb-1 leading-snug">${escapeHtml(p.title)}</h3>
        ...
        <p class="text-[#8FA3B8] text-xs leading-relaxed mb-4 flex-1">${escapeHtml(desc)}</p>
...
if (navAuth) {
  if (user) {
    ...
    navAuth.innerHTML = `<span class="text-sm text-white/70 font-medium">${escapeHtml(user.username)}</span><a href="${href}" class="text-sm bg-[#D45A1A] hover:bg-[#E8743A] text-white font-semibold px-5 py-2 rounded-lg transition-colors">Mon espace</a>`;
  } else {
